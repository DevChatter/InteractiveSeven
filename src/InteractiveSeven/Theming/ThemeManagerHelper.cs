...
namespace InteractiveSeven.Theming
{
    public class ThemeManagerHelper
    {
        ...
        {
            using var stream =
                typeof(ThemeManager).Assembly.GetManifestResourceStream(
                    "MahApps.Metro.Styles.Themes.Theme.Template.xaml");
        ...
        private static string GetGeneratorParametersJson()
        {
            using var stream =
                typeof(ThemeManager).Assembly.GetManifestResourceStream(
                    "MahApps.Metro.Styles.Themes.GeneratorParameters.json");
            using var reader = new StreamReader(stream);
            return reader.ReadToEnd();
        }
    }
